...
impl ArrowEntity {
    ...
    pub fn with_velocity(
        ...
        Self {
            ...
            state: AtomicI32::new(ArrowState::Flying as i32),
            lifetime: AtomicI32::new(0),
            damage: SyncMutex::new(10.0),
            knockback: AtomicI32::new(0),
            stuck_tick: AtomicI32::new(0),
    ...
    pub fn from_saved(
        ...
        Self::with_velocity(id, position, velocity, world)
    }
    ...
    pub fn set_knockback(&self, knockback: i32) {
        self.knockback.store(knockback, Ordering::Relaxed);
    }

    #[must_use]
    pub fn get_knockback(&self) -> i32 {
        self.knockback.load(Ordering::Relaxed)
    }

    #[must_use]
    pub fn get_state(&self) -> ArrowState {
        match self.state.load(Ordering::Relaxed) {
            0 => ArrowState::Flying,
            1 => ArrowState::Stuck,
            _ => ArrowState::Removed,
        }
    }

    fn set_state(&self, state: ArrowState) {
        ...
        *self.velocity.lock() = vel;
        ...
        if current_tick % 4 == 0 || vel.length() > 0.0 {
            ...
            if !block_state.is_air() {
                self.become_stuck(new_pos, block_state.is_solid());
                return;
            }
        }
    ...
    fn become_stuck(&self, pos: DVec3, solid: bool) {
        self.set_state(ArrowState::Stuck);
        self.stuck_tick.store(0, Ordering::Relaxed);
        ...
        *self.velocity.lock() = vel;
        ...
        if solid {
            ...
            let block_state = self
                .level()
                .map(|w| w.get_block_state(block_pos))
                .unwrap_or_default();
            if !block_state.is_air() {
                let new_pos = DVec3::new(
                    pos.x,
                    f64::from(block_pos.y()) + 1.0,
                    pos.z,
                );
                self.set_position(new_pos);
            }
    ...
    fn tick_stuck(&self) {
        let stuck_time = self.stuck_tick.fetch_add(1, Ordering::Relaxed) + 1;

        if stuck_time > STUCK_LIFETIME {
            self.set_removed(RemovalReason::Discarded);
            return;
        }
    }
    ...
    fn hit_entity(&self, target: SharedEntity) {
        ...
        let knockback = self.get_knockback();

        let actual_damage = if self.is_critical() { damage * 1.5 } else { damage };

        let source = DamageSource::entity(&vanilla_damage_types::ARROW, target.id());

        target.hurt(&source, actual_damage);

        let vel = self.velocity();
        if knockback > 0 || vel.length() > 0.0 {
            let knockback_vel = DVec3::new(vel.x * 0.1, vel.y * 0.1, vel.z * 0.1);
            let vel_target = target.velocity();
            let new_vel = DVec3::new(
                vel_target.x + knockback_vel.x,
                vel_target.y + knockback_vel.y,
                vel_target.z + knockback_vel.z,
            );
            target.set_velocity(new_vel);
        }
...
impl Entity for ArrowEntity {
    ...
    fn tick(&self) {
        ...
        match self.get_state() {
            ArrowState::Flying => self.tick_flying(),
            ArrowState::Stuck => self.tick_stuck(),
            ArrowState::Removed => {}
        }
    }
